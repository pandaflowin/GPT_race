...
from datasets import rocstories, racem, race, arc_easy, arc_challenge, NUM2CHOI
from analysis import rocstories as rocstories_analysis
from analysis import racem as racem_analysis
from analysis import race as race_analysis
from analysis import arc_easy as arc_easy_analysis
from analysis import arc_challenge as arc_challenge_analysis
...
def log():
    ...
    if submit:
        ...
        if score > best_score:
            ...
            saver.save(sess, "saved_model/rocv1")
            print("model saved")
...
transforms = {
    ...
    'race': transform_race,
    'arc-easy': transform_race,
    'arc-challenge': transform_race,
}

analyses = {
    'rocstories': rocstories_analysis,
    'racem': racem_analysis,
    'race': race_analysis,
    'arc-easy': arc_easy_analysis,
    'arc-challenge': arc_challenge_analysis,
}
...
def predict():
    ...
    dev_pred = pred_fn(iter_predict(vaX, vaM))
    if label_decoder is not None:
        predictions = [label_decoder[prediction] for prediction in predictions]
        dev_pred = [label_decoder[prediction] for prediction in dev_pred]

    path = os.path.join(submission_dir, filename)
    dev_path = os.path.join(submission_dir, f"dev-{filename}")
    ...
    with open(dev_path, 'w') as df:
        df.write('{}\t{}\n'.format('index', 'prediction'))
        for i, prediction in enumerate(dev_pred):
            df.write('{}\t{}\n'.format(i, prediction))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fixlen', action='store_true')
    ...
    if retrain or not os.path.exists(os.path.join(save_dir, desc, 'best_params.jl')):
        print("training from scratch")
        sess.run([p.assign(ip) for p, ip in zip(params[:n_transfer], init_params[:n_transfer])])
    else:
        print("load best params")
    ...
    if submit:
        #sess.run([p.assign(ip) for p, ip in zip(params, joblib.load(os.path.join(save_dir, desc, 'best_params.jl')))])
        saver.restore(sess, "saved_model/rocv1")
        pdb.set_trace()
        predict()
        if analysis:
            ...
            from ea import ea_race
            ea_race(data_dir, os.path.join(submission_dir, filenames[dataset]), os.path.join(log_dir, f'{desc}.jsonl'), text_encoder)
